...
[![Build Status](https://github.com/JiangJie/happy-opfs/actions/workflows/ci.yml/badge.svg)](https://github.com/JiangJie/happy-opfs/actions/workflows/ci.yml)
...
| **同步**     | 所有核心操作都有同步版本（如 `mkdirSync`, `readFileSync`），通过 Web Workers 实现。使用 `SyncChannel.connect`, `SyncChannel.listen`, `SyncChannel.attach`, `SyncChannel.isReady` 进行设置，`SyncChannel.disconnect` 释放通道 |
